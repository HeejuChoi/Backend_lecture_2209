...
const server = http.createServer((req, res) => {
    ...
    case '/input':
        if (method === 'GET') {             //  입력양식 화면을 사용자에게 보여줌
            fs.readFile('view/12.form.html', 'utf-8', (err, html) => {
                res.writeHead(200, {'Content-Type': 'text/html'});
                res.end(html);
            });
        } else {                            // method가 POST 인 경우
            let body = '';
            req.on('data', data => {   
                body += data;
            });
            req.on('end', () => {       // 이벤트가 끝나면 end로?
                const param = qs.parse(body);
                // 입력데이터로 필요한 작업을 수행 ex) 로그인
                // 다음 작업을 위한 화면을 보내줌
                const uid = param.uid;
                const pwd = param.pwd;
                let flag = true;
                ...
                if (flag)
                    res.end(`<h1>Re-enter the form.</h1>`);  // for문 돌고 나가면 해당 문구 출력
